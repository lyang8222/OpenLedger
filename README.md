...
> 最新进展：核心库新增**账单对账**（支付宝 CSV / 微信 xlsx / 加密 zip 解析 + 匹配引擎）、**订阅检测**（周期性扣款识别）与**自动分类**（餐饮/交通/购物/医疗等关键词规则）；App 已支持 **Face ID 应用锁**、**收支总结提醒**、**订阅到期提醒**、**手动记账**、**隐私脱敏分享卡**、**Liquid Glass 视觉打磨**、**首页漏记提醒**、**今日/本月支出小组件**、**截图后回前台导入询问** 与 **收支图表**（条形/饼图/折线/曲线/**分类占比**可切换）。
