...
> 最新进展：核心库新增**账单对账**（支付宝 CSV / 微信 xlsx / 加密 zip 解析 + 匹配引擎），已用真实账单验证——支付宝 535 笔、微信 171 笔；App 已支持 **Face ID 应用锁**、**账单总结提醒**（每日/每周/每月/每季度/每年本地通知，默认隐藏金额）与 **Liquid Glass 视觉打磨**（iOS 26 玻璃材质主按钮、玻璃标签栏与页面背景）。
