...
func testChartDataBuilder() {
    ...
    expectEqual(summaries.count, 6, "图表数据：月份数量")
    expectEqual(summaries.last?.income, Decimal(string: "200.00"), "图表数据：本月收入")
    expectEqual(summaries.last?.expense, Decimal(string: "150.50"), "图表数据：本月支出")
    let augustStart = calendar.date(from: DateComponents(year: 2026, month: 8, day: 1))!
    expectEqual(summaries[5].month, augustStart, "图表数据：本月起始")
    expectEqual(summaries[4].expense, Decimal(string: "30.00"), "图表数据：上月支出")
    expectEqual(summaries[0].expense, Decimal.zero, "图表数据：无记录月份为 0")
}
